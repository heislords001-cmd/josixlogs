...
interface SignupProps {
  onAuth: () => void;
  onEmailSignup: (username: string, email: string, password: string) => Promise<{ error?: string; needsConfirmation?: boolean }>;
  onBack: () => void;
  onLogin: () => void;
}
...
export default function Signup({ onAuth, onEmailSignup, onBack, onLogin }: SignupProps) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    setError('');
    const res = await onEmailSignup(username.trim(), email.trim(), password);
    setSubmitting(false);
